...
TEST_CASE( "Use an exp Family", "[Family]" ) {
    ...
    REQUIRE(f._transform(3.0) == exp(3.0));
    REQUIRE(f._itransform(3.0) == log(3.0));
}
...
TEST_CASE( "Use a tanh Family", "[Family]" ) {
    ...
    REQUIRE(f._transform(0.5) == tanh(0.5));
    REQUIRE(f._itransform(0.5) == atanh(0.5));
}
...
TEST_CASE( "Use a logit Family", "[Family]" ) {
    ...
    REQUIRE(f._transform(0.5) == 1.0/(1.0+exp(-0.5)));
    REQUIRE(f._itransform(0.5) == log(0.5)-log(1.0-0.5));
}