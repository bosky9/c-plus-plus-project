...
namespace cppoptlib::solver {
    ...
    auto GetDefaultStepCallback() {
        return [](const function::State<scalar_t, vector_t, hessian_t>& function_state,
            ...
            std::cout << "  gradient    " << function_state.gradient.transpose() << std::endl;
            ...
            std::cout << "  condition_hessian " << solver_state.condition_hessian << std::endl;
        };
    }

    template<class scalar_t, class vector_t, class hessian_t>
    auto GetEmptyStepCallback() {
        return [](const function::State<scalar_t, vector_t, hessian_t>&, const State<scalar_t>&) {};
    }
    ...
    public:
        ...
        using callback_t = std::function<void(const typename function_t::state_t&, const state_t&)>;
    ...
    public:
        explicit Solver(const State<scalar_t>& stopping_state = DefaultStoppingSolverState<scalar_t>(),
                        callback_t step_callback              = GetDefaultStepCallback<scalar_t, vector_t, hessian_t>())
            : stopping_state_(stopping_state), step_callback_(std::move(step_callback)) {}
        ...
        virtual std::tuple<function_state_t, state_t> Minimize(const function_t& function,
                                                               const function_state_t& initial_state) {
        ...
        virtual function_state_t OptimizationStep(const function_t& function, const function_state_t& current,
