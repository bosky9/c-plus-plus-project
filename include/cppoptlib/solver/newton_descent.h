...
namespace cppoptlib::solver {
    ...
    public:
        ...
        function_state_t OptimizationStep(const function_t& function, const function_state_t& current,
            ...
            const hessian_t hessian       = *(current.hessian) + safe_guard * hessian_t::Identity(dim_, dim_);

            const vector_t delta_x = hessian.lu().solve(-current.gradient);
            const scalar_t rate = linesearch::Armijo<function_t, Superclass::Order>::Search(current, delta_x, function);
