...
namespace cppoptlib::solver {
    ...
    public:
        ...
        void InitializeSolver(const function_state_t& initial_state) override {
            ...
            inverse_hessian_ = hessian_t::Identity(initial_state.x.rows(), initial_state.x.rows());
        }
        ...
        function_state_t OptimizationStep(const function_t& function, const function_state_t& current,
            ...
            const scalar_t rate = linesearch::MoreThuente<function_t, 1>::Search(current, search_direction, function);
            ...
            inverse_hessian_ = inverse_hessian_ -
                               rho * (s * (y.transpose() * inverse_hessian_) + (inverse_hessian_ * y) * s.transpose()) +
