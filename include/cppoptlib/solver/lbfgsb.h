...
namespace cppoptlib::solver {
    ...
    public:
        ...
        Lbfgsb(const vector_t& lower_bound, const vector_t& upper_bound,
               const State<scalar_t>& stopping_state         = DefaultStoppingSolverState<scalar_t>(),
               typename Superclass::callback_t step_callback = GetDefaultStepCallback<scalar_t, vector_t, hessian_t>())
            : Solver<function_t>{stopping_state, std::move(step_callback)}, lower_bound_{lower_bound},
        ...
        function_state_t OptimizationStep(const function_t& function, const function_state_t& current,
            ...
            const scalar_t rate = linesearch::MoreThuente<function_t, 1>::Search(current, subspace_min - current.x,
                                                                                 function, alpha_init);
            ...
            if (test > 1e-7 * new_y.squaredNorm()) {
                ...
                // STEP 7:
                theta_ = (scalar_t) (new_y.transpose() * new_y) / (new_y.transpose() * new_s);
                W_     = matrix_t::Zero(y_history_.rows(), y_history_.cols() + s_history_.cols());
                ...
                MM << D, L.transpose(), L, ((s_history_.transpose() * s_history_) * theta_);
                M_ = MM.inverse();
            }
    ...
    private:
         ...
         */
        static std::vector<int> SortIndexes(const std::vector<std::pair<int, scalar_t>>& v) {
        ...
        void GetGeneralizedCauchyPoint(const function_state_t& current, vector_t* x_cauchy, dyn_vector_t* c) const {
            ...
            *c = dyn_vector_t::Zero(W_.cols());
            ...
            // -
            // p^scalar_t*M*p
            scalar_t f_doubleprime = (scalar_t) (-1.0 * theta_) * f_prime - p.dot(M_ * p); // (O(m^2) operations)
            ...
            while ((dt_min >= dt) && (i < dim_)) {
                ...
                *c += dt * p;
                ...
                f_prime += dt * f_doubleprime + current.gradient(b) * current.gradient(b) +
                           theta_ * current.gradient(b) * zb - current.gradient(b) * wbt.transpose() * (M_ * *c);
                f_doubleprime += scalar_t(-1.0) * theta_ * current.gradient(b) * current.gradient(b) -
                                 scalar_t(2.0) * (current.gradient(b) * (wbt.dot(M_ * p))) -
                                 current.gradient(b) * current.gradient(b) * wbt.transpose() * (M_ * wbt);
            ...
            (*x_cauchy)(sorted_indices) = current.x(sorted_indices) + t_old * d(sorted_indices);
         ...
         */
        scalar_t FindAlpha(const vector_t& x_cp, const dyn_vector_t& du, const std::vector<int>& free_variables) const {
            ...
            for (unsigned int i = 0; i < n; i++) {
                if (du(i) > 0) {
                    alphastar = std::min<scalar_t>(
                            alphastar, (upper_bound_(free_variables.at(i)) - x_cp(free_variables.at(i))) / du(i));
                } else {
                    alphastar = std::min<scalar_t>(
                            alphastar, (lower_bound_(free_variables.at(i)) - x_cp(free_variables.at(i))) / du(i));
                }
            }
            return alphastar;
        }

        vector_t SubspaceMinimization(const function_state_t& current, const vector_t& x_cauchy,
            ...
            const matrix_t WZ        = W_(free_variables_index, Eigen::indexing::all).transpose();

            const vector_t rr = (current.gradient + theta_ * (x_cauchy - current.x) - W_ * (M_ * c));
            // r=r(free_variables);
            ...
            const dyn_vector_t du = -theta_inverse * r - theta_inverse * theta_inverse * WZ.transpose() * v;
            // STEP: 7
            ...
            for (int i = 0; i < free_var_count; i++) {
                subspace_min(free_variables_index[i]) = subspace_min(free_variables_index[i]) + dStar(i);
            }
            return subspace_min;
        }
