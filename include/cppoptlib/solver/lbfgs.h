    ...
    public:
        ...
        function_state_t OptimizationStep(const function_t& function, const function_state_t& current,
            ...
            const scalar_t relative_eps =
                    static_cast<scalar_t>(absolute_eps) * std::max<scalar_t>(scalar_t{1.0}, current.x.norm());
            ...
            for (int i = k - 1; i >= 0; i--) {
                ...
                const scalar_t rho = 1.0 / x_diff_memory_.col(i).dot(grad_diff_memory_.col(i));
            ...
            for (int i = 0; i < k; i++) {
                // beta <- rho_i * y_i^T * r
                const scalar_t rho  = 1.0 / x_diff_memory_.col(i).dot(grad_diff_memory_.col(i));
                const scalar_t beta = rho * grad_diff_memory_.col(i).dot(search_direction);
            ...
            const scalar_t rate =
                    linesearch::MoreThuente<function_t, 1>::Search(current, -search_direction, function, alpha_init);

            const function_state_t next = function.Eval(current.x - rate * search_direction, 1);
