    ...
    public:
        ...
        function_state_t OptimizationStep(const function_t& function, const function_state_t& current,
            ...
            if (state.num_iterations == 0) {
                ...
                const double beta =
                        current.gradient.dot(current.gradient) / (previous_.gradient.dot(previous_.gradient));
                search_direction_ = -current.gradient + beta * search_direction_;
            }
            previous_ = current;

            const scalar_t rate = linesearch::Armijo<function_t, 1>::Search(current, search_direction_, function);
