...
namespace cppoptlib::solver::linesearch {
    ...
    public:
         ...
         * @return step-width
         */
        static scalar_t Search(const state_t& state, const vector_t& search_direction, const function_t& function,
    ...
    public:
        ...
        static scalar_t Search(const state_t& state, const vector_t& search_direction, const function_t& function) {
            ...
            const scalar_t f_in    = state.value;
            const scalar_t cache   = c * state.gradient.dot(search_direction) +
                                   0.5 * c * c * search_direction.transpose() * ((*state.hessian) * search_direction);
