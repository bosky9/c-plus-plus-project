...
namespace cppoptlib::solver::linesearch {
    ...
    public:
        ...
        static scalar_t Search(const state_t& state, const vector_t& search_direction, const Function& function,
        ...
        static int cvsrch(const Function& function, vector_t* x, scalar_t f, vector_t* g, scalar_t* stp,
                          const vector_t& s) {
            ...
            while (true) {
                ...
                *x = wa + *stp * s;
                ...
                if (stage1 & (f <= ftest1) & (dg >= std::min<scalar_t>(ftol, gtol) * dginit))
                    stage1 = false;
        ...
        static int cstep(scalar_t& stx, scalar_t& fx, scalar_t& dx,    // NOLINT
            ...
            if ((brackt & ((stp <= std::min<scalar_t>(stx, sty)) | (stp >= std::max<scalar_t>(stx, sty)))) |
            ...
            if (fp > fx) {
                ...
                if (brackt) {
                    ...
                    scalar_t gamma = s * sqrt((theta / s) * (theta / s) - (dy / s) * (dp / s));
                    if (stp > sty)
                        gamma = -gamma;
            ...
            if (brackt & bound) {
                if (sty > stx) {
                    stp = std::min<scalar_t>(stx + static_cast<scalar_t>(0.66) * (sty - stx), stp);
                } else {
                    stp = std::max<scalar_t>(stx + static_cast<scalar_t>(0.66) * (sty - stx), stp);
                }
            }
