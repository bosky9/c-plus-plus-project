...
namespace cppoptlib::utils {
    ...
    void ComputeFiniteGradient(const function_t& function, const typename function_t::vector_t& x0,
                               typename function_t::vector_t* grad, const int accuracy = 0) {
    ...
    template<class function_t>
    void ComputeFiniteHessian(const function_t& function, const typename function_t::vector_t& x0,
                              typename function_t::hessian_t* hessian, int accuracy = 0) {
        using scalar_t = typename function_t::scalar_t;
        using vector_t = typename function_t::vector_t;
        using index_t  = typename function_t::index_t;
    ...
    template<class function_t>
    bool IsGradientCorrect(const function_t& function, const typename function_t::vector_t& x0, int accuracy = 3) {
        ...
        for (index_t d = 0; d < D; ++d) {
            scalar_t scale =
                    std::max(static_cast<scalar_t>(std::max(fabs(actual_gradient[d]), fabs(expected_gradient[d]))),
                             scalar_t(1.));
    ...
    template<class function_t>
    bool IsHessianCorrect(const function_t& function, const typename function_t::vector_t& x0, int accuracy = 3) {
        ...
        for (index_t d = 0; d < D; ++d) {
            for (index_t e = 0; e < D; ++e) {
                scalar_t scale = std::max(
                        static_cast<scalar_t>(std::max(fabs(actual_hessian(d, e)), fabs(expected_hessian(d, e)))),
                        scalar_t(1.));
                if (fabs(actual_hessian(d, e) - expected_hessian(d, e)) > tolerance * scale)
                    return false;
            }
        }
        return true;
