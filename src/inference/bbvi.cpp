...
BBVI::BBVI(std::function<double(Eigen::VectorXd)> neg_posterior, const std::vector<Normal>& q, int sims,
    ...
    : _neg_posterior{std::move(neg_posterior)}, _q{q}, _sims{sims}, _printer{true}, _optimizer{std::move(optimizer)}, _iterations{iterations},
...
CBBVI::CBBVI(std::function<double(Eigen::VectorXd)> neg_posterior, std::function<double(Eigen::VectorXd)> log_p_blanket,
    ...
    : BBVI{std::move(neg_posterior), q, sims, std::move(optimizer), iterations, learning_rate, record_elbo, quiet_progress},
      _log_p_blanket{std::move(log_p_blanket)}
{}

CBBVI::CBBVI(const CBBVI& cbbvi) :
      _log_p_blanket{cbbvi._log_p_blanket},
      BBVI{cbbvi._neg_posterior, cbbvi._q, cbbvi._sims, cbbvi._optimizer, cbbvi._iterations, cbbvi._learning_rate, cbbvi._record_elbo, cbbvi._quiet_progress}
{
    /*
    if (_optimizer == "ADAM")
...
CBBVI::CBBVI(CBBVI&& cbbvi) noexcept :
      BBVI(std::move(cbbvi))
{
    _log_p_blanket = cbbvi._log_p_blanket;
...
BBVIM::BBVIM(std::function<double(Eigen::VectorXd, int)> neg_posterior,
    ...
    : BBVI{std::function<double(Eigen::VectorXd)>(),
           ...
           record_elbo,
           quiet_progress},
      _neg_posterior{std::move(neg_posterior)}, _full_neg_posterior{std::move(full_neg_posterior)}, _mini_batch{mini_batch} {}

BBVIM::BBVIM(const BBVIM& bbvim) :
      BBVIM{bbvim._neg_posterior, bbvim._full_neg_posterior, bbvim._q, bbvim._sims, bbvim._optimizer, bbvim._iterations, bbvim._learning_rate, bbvim._mini_batch, bbvim._record_elbo, bbvim._quiet_progress}
{}
